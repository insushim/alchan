...
## 13. 잔여 6건 처리 — 2026-08-12

§12 에서 "별건으로 남긴다"고 했던 기존 코드의 구조적 결함을 전부 처리했다.
1차 커밋(`99265c1`)은 이미 검증·배포됐고, 이건 그 위에 얹는 두 번째 라운드다.

### 고친 것

**C6-b 재산세 배치 절벽** — 학생당 3쓰기(현금 + 순자산세로그 + 보유세로그)라 166명 안팎에서
500 한도를 넘어 **그 학급 전체 징수가 실패**했다. 주급과 같은 방식으로 분할 커밋 + 학생 단위 마커
`lastWeeklyTaxWeekKey`(userUpdate 에 필드 하나 = 쓰기 증가 0).
- 부수 효과가 하나 있는데 오히려 개선이다: **force 재실행이 비로소 안전해졌다.**
  종전 force 는 "이 학급을 다시 걷는다" = 이미 낸 학생까지 재과세였다. 이제 못 걷은 학생만 걷는다.
  문서화된 복구 경로(under-tax → 교사 force)가 실제로 쓸 수 있는 것이 됐다.

**C8 월세 멱등성** — 매물별 트랜잭션은 있었지만 마커가 없어, 전역 락이 실패로 풀린 뒤
재실행되면 이미 걷은 매물을 또 걷었다. 트랜잭션 **첫 읽기**로 매물 문서를 읽어 `lastRentWeekKey` 를
확인하고, 차감과 **같은 트랜잭션**에 기록한다. (읽기 3개가 전부 쓰기보다 앞 — Firestore 제약 충족 확인.)

**C11 economicEvents 비원자성** — 환급·보너스·패널티 셋 다 학생 배치를 전부 커밋한 **뒤**
관리자 cash 를 별도 쓰기로 갱신했다. 그 쓰기가 실패하면 환급·보너스는 **무담보 발행**,
패널티는 **학생 돈만 소각**이다. 관리자 증감을 학생 배치 안으로 옮겨, 커밋된 배치는 언제나 수지가 맞는다.

**C11-b 나눗셈 잔여 소각** — 환급은 학생에게 `floor(총액/N) × N` 을 주면서 교사에게선 `총액`을 뺐다.
차액이 아무에게도 안 가고 사라졌다. 이제 실제 지급 합계만 차감한다.
(표시 문구도 정확해졌다 — 종전엔 소각분까지 "환급"이라 보여 줬다.)

**C12 순자산 빈 catch** — `catch (_) {}` 3개가 조회 실패를 0 으로 만들어 **읽지도 못한 값으로 과세**했다.
throw 로 바꾸고 호출자가 학생 단위로 잡아 **그 학생을 과세 대상에서 제외**한다.
추정치로 돈을 걷느니 한 번 거르는 게 낫다. (🔬 호출자는 `executeCashPenalty` 하나뿐임을 전수 확인.)

**C3 배당 마커 소실** — 마커 위치가 이 리뷰에서만 두 번 바뀌었다. 둘 다 교차검증에서 뚫려서다:
① 스칼라 → 백필이 과거로 덮어씀(C2) ② 보유문서 위의 월별 맵 → 전량매도·재매수 시 문서가 새로 생겨 소실(C3).
이제 **사용자·월 원장** `users/{uid}/dividendLedger/{monthKey}` 의 `paid[stockId]` 다.
달마다 문서가 갈리고, 보유 문서를 지웠다 만들어도 원장은 그대로다.
비용은 보유가 있는 사용자당 월 1읽기(88명 규모에서 무시 가능). rules 는 `write: if false`.

**HIGH3 내부 catch** — 분기별 try/catch 는 **부분 실패 허용**이 의도라 유지하되, 실패를 모아
마지막에 던진다. 주가가 안 갱신되면 학생이 **옛 가격으로 거래**하므로 조용히 지나가면 안 된다.

**LOW** — `volatility || 기본값` 이 `0` 을 falsy 로 보고 덮었다(0 을 정식으로 통과시켜 놓고선).

### 이 라운드에서 **내가 만든 구멍 두 개**를 잡았다

**① 세금 회피 통로(배포 전 차단)** — 새 마커를 user 문서에 넣었는데, users update 규칙은
**블록리스트** 방식이라 목록에 없는 필드는 학생이 자유롭게 쓴다.
즉 학생이 자기 문서에 `lastWeeklyTaxWeekKey = 이번주` 를 심으면 **징수를 건너뛴다.**
🔬 확인 후 블록리스트에 추가했다(+ lastSalaryDate·lastNetSalary·lastGrossSalary·lastTaxAmount).
- 1차 배포분의 `lastSalaryWeekKey` 도 같은 성질이지만 **자해만 가능**하다(자기 주급을 포기).
  남의 것은 못 건드리고 두 번 받을 수도 없다.
- 교훈: **서버 판정에 쓰는 필드를 user 문서에 새로 얹을 때는 반드시 블록리스트를 같이 본다.**
  블록리스트 방식은 "새 필드는 기본 허용"이라 조용히 열린다.

**② 팝업이 안 닫히는 회귀(넣기 전 차단)** — 그 봉인에 `pendingTaxSummary` 를 같이 넣었는데,
🔬 학생이 주간 세금 팝업을 "확인"으로 닫을 때 **본인이 `deleteField()` 로 지우는 필드**였다
(`WeeklyTaxSummaryPopup.js:26`). 넣었으면 팝업이 영영 안 닫힌다. 되돌리고 ALLOW 카나리아로 바꿨다.
위조해 봐야 자기 화면 안내 문구가 바뀔 뿐 징수액은 서버가 계산한다.

### 검증

| 검사 | 결과 |
|---|---|
| Firestore Rules `:test` | **71/71** (신규 7 — 마커 봉인 4 · 배당원장 3) |
| vitest | 383/383 |
| 재산세 배치 산수 | 학생당 3 · 학급마감 3 — 예약값과 일치 확인 |
| 배당 배치 산수 | 보유당 5쓰기로 증가, 1만 보유·200학급에서도 최대 448 ops |
| 월세 트랜잭션 | 읽기 3개가 전부 쓰기보다 앞 (Firestore 제약) |
| 반환값 계약 변경 | 소비처는 표시 문구 2곳뿐 — 오히려 정확해짐 |
| 블록리스트 회귀 | 추가 7필드를 src 전수 대조 → 1건(pendingTaxSummary) 발견·제외 |

### 이 라운드의 교차검증 — "같은 규약, 빠진 형제" 가 **또** 나왔다

두 곳에서 나왔고, 발견 경로가 서로 달랐다.

**① `executeTaxExtra` — 내 전수 감사가 잡았다.**
경제이벤트 네 함수 중 셋을 고치고 하나를 빼먹었다. 코드를 다시 읽어서가 아니라
**커버리지 표를 기계로 뽑아서** 찾았다("돈을 움직이는 작업 × 락/마커/원자성" 격자).
세 번 같은 실수를 한 뒤에야 표를 만든 게 늦었다 — 규약을 세울 때 적용 대상 목록을 **먼저** 만들었어야 한다.

**② 재산세 국고 입금 — Gemini 가 잡았다(CRITICAL).**
같은 PR 에서 economicEvents 3함수에 "청크별 국고 입금" 규약을 세워 놓고,
**형제 함수인 재산세가 그 규약을 어기고 있었다.** 학생 차감은 배치별로 나가는데
국고 입금만 맨 끝 한 번이라, 중간 배치 커밋 후 죽으면 걷은 돈이 국고에 안 들어가고
재실행은 학생 마커에 막혀 그만큼이 **영구 소각**된다.
→ `creditTreasury()` 로 flush 마다 그 배치 몫을 함께 내보낸다. 🔬 5,000명에서도 최대 449 ops.

### Gemini 지적 2건은 기각했다 — 둘 다 실측으로

**"`set({paid:{A:true}}, {merge:true})` 가 paid 맵을 통째로 덮어쓴다"** → 오탐.
🔬 Firestore 라이브러리가 실제로 만드는 필드 마스크를 뽑아 확인했다:
```
DocumentMask.fromObject({paid:{stockA:true}, monthKey:"2026-08"})
  → {"fieldPaths":["monthKey","paid.stockA"]}
```
leaf 경로로 펼쳐지므로 다른 종목 키는 보존된다. `update()` 의 얕은 덮어쓰기 함정을
`set(merge:true)` 에 잘못 적용한 것이다.

**"`realEstateProperties` 의 `lastRentWeekKey` 가 클라 조작에 열려 있다"** → 오탐.
🔬 그 컬렉션은 `allow write: if isClassAdmin(classCode)` 로 **전체가 교사·슈퍼관리자 전용**이다.
학생은 필드 단위 이전에 쓰기 자체가 불가능하다. users 문서의 "본인은 넓게 열고 필드로 좁힌다"
패턴을 다른 컬렉션에 일반화한 것으로 보인다.

### 최종 커버리지 (실측 격자)

| 작업 | 실행 락 | 단위 멱등 마커 | 반대계정 같은 커밋 |
|---|---|---|---|
| 주급 (자동+수동) | ✅ | `lastSalaryWeekKey` | ✅ |
| 재산세 (자동+수동) | ✅ | `lastWeeklyTaxWeekKey` | ✅ (이번에 수정) |
| 월세 (자동+수동) | ✅ | `lastRentWeekKey` | ✅ (매물별 트랜잭션) |
| 적금 (자정+수동) | ✅ | `lastDepositDate` | ✅ (트랜잭션) |
| 배당 (자동+수동) | ✅ | `dividendLedger` | ✅ |
| 경제이벤트 4종 | — (교사 트리거) | — | ✅ (4/4, 이번에 통일) |
| 주급 회수 (수동) | ✅ | 의도적 미적용 — 사람이 판단 |

**Tier-0 게이트 6항목 PASS · Rules 71/71 · vitest 383/383.**

### 3계열 재검증 결과 — 채택 13 · 기각 4 · 해당없음 1

| 계열 | 판정 | 결과 |
|---|---|---|
| codex | REQUEST_CHANGES | CRITICAL 7 · HIGH 2 · MEDIUM 1 → **6 채택 · 1 해당없음** |
| Gemini | REQUEST_CHANGES | CRITICAL 2 · WARNING 1 → **1 채택 · 2 기각** |
| Claude | REQUEST_CHANGES | MEDIUM 1 · WARNING 2 · LOW 1 → **4 채택** |

**세 계열이 서로 다른 것을 잡았다.** 겹친 건 하나도 없다 — 렌즈를 나눈 게 실제로 작동했다.

#### 채택 — 이번에도 **내 수정 자체의 결함**이 다수였다

**월세 stale 참조 (codex CRITICAL)** — 트랜잭션 안에서 신선한 문서를 읽어 놓고
**마커만 그걸 쓰고 세입자·임대료·집주인은 stale 한 바깥 변수**를 썼다. 그러면 트랜잭션을 쓰는 의미가 없다.
계약이 바뀌면 옛 세입자에게서 옛 임대료를 걷고 새 계약 문서에 "이번 주 완료" 도장을 찍는다
→ 새 세입자의 월세가 영구 누락. **전부 `fresh` 기준으로 바꿨다.**

**그 전환이 또 불완전했다 (Claude MEDIUM)** — "이 아래는 **전부** fresh 를 쓴다"고 주석까지 달아 놓고
집주인 거래로그 5곳을 stale 로 남겼다. 금액·수취인은 맞지만 집주인 통장에 **옛 세입자 이름**이
영구 기록된다. 이 저장소는 거래로그 정확성을 안전망 1항으로 못 박고 있다.

**force 가 진행중 락을 관통 (codex CRITICAL)** — 내 `decideClaim` 설계 오류.
`force` 는 "완료된 걸 다시 돈다"는 뜻이지 "다른 실행과 나란히 돈다"가 아니다.
교사가 수동 징수 버튼을 연달아 두 번 누르면 두 실행이 같은 스냅샷을 보고 **둘 다 걷었다.**
이제 force 도 **진행 중인** 락은 존중한다(완료·실패만 관통).

**월세 수취인 없으면 소각 (codex CRITICAL)** — `ownerRef` 가 null 인데 세입자 차감·마커는 그대로 커밋.
받을 사람이 없으면 걷지 않는다.

**경제이벤트 재시도 이중지급 (codex CRITICAL)** — 완료 마커가 실행 **후**인데 hourlyV2 가 평일 8~17시
**매시간** 돈다. 부분 실패 후 같은 날 최대 9번 재실행되고 그때마다 이미 받은 학생에게 또 지급된다.
→ claim-before 로 전환. 랜덤 연출 이벤트라 하루 거르는 쪽이 이중지급보다 낫다(재산세와 같은 판단).

**학급 공통 조회 fail-open (codex CRITICAL)** — 학생 단위 방어(C12)만으론 부족했다.
CentralStocks·부동산 조회가 실패하면 **빈 맵으로 전원 과세**된다. 공통 입력은 fail-closed 로.
`executeTaxExtra` 의 학생 단위 조회 3곳도 같은 기준으로 정리(못 읽은 학생은 제외).

**재산세 국고 입금이 맨 끝 한 번 (Gemini CRITICAL)** — **같은 PR 에서 막 세운 규약을 형제 함수가 어겼다.**
economicEvents 3함수는 청크별로 국고에 입금하는데 재산세만 학생 차감은 배치별, 국고 입금은 끝에 한 번.
중간 커밋 후 죽으면 걷은 돈이 국고에 안 들어가고 마커 때문에 보정도 안 된다 → **영구 소각.**
→ `creditTreasury()` 로 청크마다. **국고 통계도 같이**(Claude LOW — 통계만 끝에 쓰면 대시보드가 조용히 틀려진다).

**블록리스트 5필드 (codex HIGH)** — `dailySpinDate`/`dailySpinCount`(추첨 한도 무한 우회),
`lastIncomingTransferAt`(24h 대출상환 제한 우회), `lastLoanRepaidAt`(24h 재대출 제한 우회),
`totalSalaryReceived`(백필 감사로그 오염). 🔬 전부 서버 판정·클라 write 0건 확인.

**하위 함수 fail-soft (codex HIGH)** — `updateExchangeRate` 는 실패해도 `{updated:false}` 를 돌려주고,
종목 시세 부분 실패는 `failed++` 만 하고 정상 반환한다. 그래서 내 재throw 검사에 안 걸렸다.
하위 함수의 반환 계약을 바꾸는 대신 **호출부가 결과를 검사**하도록 했다(fail-soft 자체는 의도된 설계다).
학급별 경제이벤트 실패도 `classErrors` 로 올려 `branchErrors` 까지 전달.

**죽은 심볼 (Claude WARNING)** — `totalNeeded`(반환식이 바뀌며 고아), `monthFieldKey`(원장 전환으로 소비처 상실).
`economicEvents.js` 는 파일 전체 `eslint-disable` 이라 린트가 못 잡는 것도 확인.

#### 기각 — 전부 **실측**으로 판정했다(추론으로 반박하지 않았다)

**"`set({paid:{A:true}}, {merge:true})` 가 paid 맵을 통째로 덮어쓴다"(Gemini)** → 오탐.
🔬 Firestore 라이브러리가 실제로 만드는 필드 마스크를 뽑았다:
`DocumentMask.fromObject({paid:{stockA:true}, monthKey:"..."})` → `{"fieldPaths":["monthKey","paid.stockA"]}`.
leaf 로 펼쳐지므로 다른 종목 키는 보존된다. `update()` 의 얕은 덮어쓰기 함정을 `set(merge:true)` 에 적용한 오해.

**"`realEstateProperties` 의 마커가 클라 조작에 열려 있다"(Gemini)** → 오탐.
🔬 그 컬렉션은 `allow write: if isClassAdmin(classCode)` 로 **전체가 교사 전용**이다.
users 문서의 "본인은 넓게 열고 필드로 좁힌다" 패턴을 다른 컬렉션에 일반화한 것.

**"배당 원장 전환에 마이그레이션이 없다"(codex)** → 해당 없음.
🔬 라이브 portfolio 23건 전수 조회 — 구 마커(`lastDividendMonthKey`·`dividendPaidMonths`) **0건**.
새 코드로 배당이 한 번도 안 돌았으므로 이관할 상태 자체가 없다.

**"재산세 국고 유실"(codex CRITICAL #1)** → Gemini 지적으로 이미 해소된 상태였다(옛 스냅샷 기준 보고).

#### 이 라운드의 교훈

같은 실수가 **다섯 번째**로 나왔다 — "새 규약을 세우고 적용 대상 하나를 빠뜨린다".
이번엔 `executeTaxExtra`(내가 격자로 발견) · 재산세 국고입금(Gemini) · 월세 로그 필드(Claude).
**규약을 세울 때 적용 대상 목록을 먼저 만들지 않은 것이 근본 원인이다.**
커버리지 격자를 기계로 뽑고 나서야 빠진 게 보였다 — 다음엔 그걸 **먼저** 만든다.
