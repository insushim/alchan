...
- 진행 중 = **P1-4a 보상 없는 이관**(2026-08-22 · 아래 「P1-4a」). 게이트 ② 는 닫혔고 ③ 은 사람 확인 대기.
...
#### 🟡 아직 안 한 것
- **앱 쪽(gugu-guardians) 구현.** 토큰 수신·JWKS 검증·`sub` 로 학생 식별. 계획서 순서상
  `migrate` 를 켜고 관찰하는 게 먼저라 이번 라운드에서 뺐다. 지금 토큰을 보내도 앱은 무시한다.
- **`migrate` 스위치를 아직 안 켰다.** 클라 다리가 배포된 뒤에 켠다 — 순서를 뒤집으면
  토큰이 나가는데 부를 화면이 없다(P1-5 에서 겪은 것과 같은 모양).
