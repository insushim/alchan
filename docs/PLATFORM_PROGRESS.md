...
- 방금 끝난 것 = **P1-4b 앱 쪽 구현**(2026-08-22 · 커밋됨, 앱 배포 전). 아래 「P1-4b」
- 다음 = 학습기록 켜기(`stats-on`) 또는 게이트 ③(경보 수신 — 사람만 닫을 수 있다).
...
| ③ 경보 수신 | 🟡 배선 완료·**수신 확인만 남음** | 채널 `enabled` · 인증 차단 사유 없음(API 실측 2026-08-22). 전달 영수증은 API 가 안 준다 — 수신함은 사람이 본다 |
...
#### 🔬 2026-08-22: API 로 좁힐 수 있는 데까지 좁혔다 (그래도 사람이 필요하다)
`monitoring-alerts.mjs list` 가 채널 상태를 `?` 로 찍길래 API 를 직접 물었다.
- 채널은 `enabled: true` 이고 이메일 주소도 맞다.
- `verificationStatus` 는 **필드 자체가 응답에 없다** — 필드마스크로 명시 요청해도 없다.
  proto3 JSON 은 enum 기본값(`VERIFICATION_STATUS_UNSPECIFIED`)을 생략하므로, 이건
  "인증 대기"가 아니라 **"인증이 적용되지 않는 채널"** 로 읽는 것이 맞다(구글 문서상
  UNSPECIFIED 는 "인증을 지원하지도 요구하지도 않는 채널"을 포함한다).

→ 즉 **알려진 차단 사유는 없다.** 그렇다고 "도착했다"가 증명되는 건 아니다 —
   Monitoring API 는 **전달 영수증을 주지 않는다.** 여기까지가 기계가 답할 수 있는 끝이고,
   마지막 한 칸은 여전히 수신함이다. 🔴 **"차단 사유가 없다"와 "도착했다"를 같은 말로
   쓰지 않는다** — 그렇게 뭉개면 게이트가 닫힌 척하게 된다.
...
### 🎮 P1-4b 앱이 토큰을 받는다 (2026-08-22) — 저장소 `insushim/gugu-guardians`

| 파일 | 무엇 |
|---|---|
| `src/net/aap.ts` (신규) | 규약 §3 구현 — fragment 읽기·JWKS 검증·`sub` 로 식별 |
| `src/ui/aapBadge.ts` (신규) | 4초 배지. `doc` 주입식이라 jsdom 없이 테스트된다 |
| `src/ui/aapStatus.ts` (신규) | 설정 화면 상태줄 — 배지를 놓쳐도 확인할 자리 |
| `tests/aap.spec.ts` (신규) | 37케이스 |
| `src/main.ts` · `src/ui/screens.ts` · `src/style.css` | 배선 |

**실측**: 353 테스트(신규 37) · **변이 21종 전부 검출** · tsc·밸런스·스프라이트 게이트 통과 · 빌드 exit 0.

#### 이 게임의 성질을 규약이 바꾸지 않게 했다
구구성 수호대는 **로그인이 없는 게임**이다. 알찬을 안 거쳐도 전부 그대로 되고, 검증이
실패해도 게임은 돈다. 신원은 **메모리에만** 둔다 — 공용 PC 에서 다음 아이가 앞 아이의
신원을 물려받으면 안 된다. 토큰은 관문이 아니라 있으면 좋은 것이다.

#### 🔬 실제 브라우저로 잰 것 (단위 테스트만으로는 못 믿을 자리)
| 불변식 | 실측 |
|---|---|
| 미경유 부팅 | 배지 없음 · 메뉴 정상 |
| 실패해도 게임이 돈다 | `{kind:"failed"}` · `ready:true` |
| fragment 삭제 | `href` = `/?t=1` — 토큰 사라지고 쿼리는 보존 |

🔴 **여기서 또 내 측정이 거짓말할 뻔했다.** 해시만 바꿔 이동했더니 `boot()` 가 아예 안 돌아
`aap:{kind:"none"}` 이 나왔고, 나는 그걸 "fragment 삭제 실패"로 읽을 참이었다. 같은 문서 내
이동은 리로드가 아니다 — 쿼리를 붙여 진짜 로드로 다시 재고서야 정상 동작이 보였다.
**P1-4a 의 팝업 프로브와 같은 실수다: 관측 방법이 관측 대상을 안 건드리고 있었다.**

#### 🔎 FULL 교차검증 (P1-4b)

| 계열 | 판정 |
|---|---|
| Gemini | **APPROVE** (WARNING 2 + NIT 2 — 2건 채택, 2건 근거 대고 기각) |
| Claude(`code-reviewer`) | **APPROVE** (WARNING 4 — 전부 재현·채택) |
| codex(GPT) | **중단** — 콘텐츠 정책에 걸려 최종 판정 없음. 다만 **죽기 전에 발견 하나를 남겼다** |

🔴 **Gemini 가 내 주석으로 내 코드를 고발했다.** 주석엔 "게임 시작이 늦어지면 안 된다"고
써 놓고 코드는 `await identity` 로 메뉴를 막고 있었다 — 에셋이 다 받아져 진행바가 100% 를
찍어도 신원이 올 때까지 화면이 멈춘다(최악 discovery+JWKS+회전 재조회 3회 × 5초).
**주석은 의도를 적는 곳이지 알리바이가 아니다.**

🔴 **`toThrow('kid')` 가 부분일치라 검사를 통째로 지워도 초록이었다**(Claude 레인 변이 생존).
`kid` 검사를 없애면 나중에 `kid-unknown` 이 나는데, 그게 `'kid'` 를 포함하니 테스트가 통과한다.
→ 정확매치(`/^kid$/`)로 바꾸고 사유 이름도 `kid-missing` 으로 갈랐다.
**사유 이름이 서로의 접두사면 정확매치가 무너진다.**

🔴 **codex 는 죽으면서도 값을 냈다.** JWT 위조 프로브를 돌리다 사이버보안 필터에 걸려
중단됐지만, 그 전에 `base64url 의 마지막 글자는 표기가 여럿`임을 실증했다(같은 5바이트
서명에 `U·V·W·X` 4개, 전부 같은 바이트). 내 주석은 "엄격 검사로 같은 토큰의 문자열 형태가
여럿이 되는 걸 막는다"고 **단언하고 있었는데 거짓이었다.**
→ 주석을 사실로 고치고, 그 사실 자체를 테스트로 박았다. 무해한 이유도 같이 적었다 —
   재생 방어는 서버가 `aapRewardSessions/{jti}` 로 하지 **토큰 문자열로 하지 않는다.**
   위험한 건 "여기서 문자열로 중복을 거르기 시작하는 것"이라, 그걸 주석이 막는다.
⚠️ 검증자가 **정책 필터로 죽을 수 있다**는 건 처음 겪었다. 침묵과 구별되지 않으므로
   출력 파일을 직접 열어야 안다(이번 라운드에만 전달 실패가 3번 났다).

#### 내가 스스로 잡은 테스트 결함 둘
- fragment 순서 테스트가 **만료된 토큰**으로 돌아 실패 경로에서만 통과하고 있었다 —
  순서는 맞혔지만 **성공 경로를 한 번도 안 밟는** 테스트였다.
- 키 회전 테스트가 `json: async () => ({ keys: served })` 로 **변수를 닫아 잡아**, 첫 조회가
  이미 새 키까지 돌려줬다. 재조회 경로를 한 번도 안 밟았고 **변이 생존이 알려 줬다.**
  🔴 **지연 평가하는 목은 "무엇을 주는지"만이 아니라 "언제 읽히는지"까지 봐야 한다.**

#### 🔴 배포하고 나서 안 것 — **초록불이 거짓말을 하고 있었다**

P1-4b 를 푸시했더니 CI 가 초록불을 냈다. 그런데 학생 사이트는 그대로였다.

| 주소 | 번들 | 뜻 |
|---|---|---|
| `insushim.github.io/gugu-guardians/` | `index-B44uVdQg.js` | 학교망이 **막는** 주소. CI 가 여기로만 나간다 |
| `gugu-guardians.pages.dev` | `index-CyoNdETL.js` | **학생이 여는** 주소. 어제 손으로 올린 것 그대로 |

`wrangler pages project list` → **`Git Provider: No`**. 즉 이 프로젝트는 git 연동도 CI 도 없고,
누가 `wrangler pages deploy` 를 **손으로 쳐야만** 갱신된다. 어제 "Cloudflare 로 옮겼다"는
커밋은 URL 과 CORS 만 옮겼고 **파이프라인은 두고 갔다.**

→ 오늘 것을 먼저 손으로 올려 학생에게 보냈고(반영 확인), CI 에 `cloudflare` 잡을 넣었다.
   시크릿(`CLOUDFLARE_API_TOKEN`·`CLOUDFLARE_ACCOUNT_ID`)이 없으면 **경고를 남기고 건너뛴다** —
   조용히 실패하면 지금과 똑같은 상태가 그대로 반복된다. github.io 배포는 일부러 남겼다.

🔴 **이 라운드에서 같은 교훈이 세 겹으로 나왔다.**
   ① 앱을 옮겼는데 **포털의 주소**를 안 고쳤다(P1-4a)
   ② 앱을 옮겼는데 **등재 신청 페이로드**가 옛 주소였다(iwpuzzle)
   ③ 앱을 옮겼는데 **배포 파이프라인**이 안 따라갔다(여기)
   전부 "떠나는 쪽은 옮겼는데 **가리키는 쪽·보내는 쪽**은 그대로"다.
   **이사의 완료 판정은 출발지가 아니라 도착지를 가리키는 모든 것에서 해야 한다.**

✅ **크로마폴(`iwpuzzle`)도 같이 막았다**(2026-08-22). 워크플로가 아예 없었는데 —
   🔴 **게이트는 이미 있었다**: vitest 75개 · eslint · tsc(`build` 가 `tsc -b` 포함).
   즉 "게이트가 없다"가 아니라 **"게이트를 안 돌린다"** 였고, 둘은 다른 문제다.
   같은 모양의 `build` + `cloudflare` 잡을 넣었고 첫 실행에서 셋 다 통과·배포는 경고 후 skipped.
   (`package.json` 의 `deploy: gh-pages -d dist` 는 **차단되는 주소로 나가는 옛 경로**라 표시해 뒀다.)

📌 **두 저장소 모두 시크릿 두 개가 들어와야 자동 배포가 켜진다** —
   `CLOUDFLARE_API_TOKEN`(Edit Cloudflare Workers 템플릿) · `CLOUDFLARE_ACCOUNT_ID`.
   그때까지는 경고만 남기고 건너뛴다. 각 README 에 수동 명령을 적어 뒀다.

#### 🟡 남은 것
- **별명이 실제로는 안 온다.** 배지·상태줄이 `nick` 을 쓸 수 있게 돼 있지만 라이브 정책의
  `allowedClaims` 가 비어 있다. 아이에겐 "알찬 · 별똥별"이 "연결됨"보다 훨씬 명확하지만,
  학생 데이터를 앱에 하나 더 주는 결정이라 **사용자 판단으로 남긴다**(학생이 스스로 정한
  닉네임만 나가고 실명은 안 나간다 — 서버가 `hasSetNickname` 으로 가른다).
- **원본 토큰을 안 들고 있다**(Gemini WARNING 2). 다음 단계에서 `grantAppReward` 를 부르려면
  필요하다 — 지금 넣으면 아무도 안 쓰는 필드라 그때 넣는다. 필드 하나 추가라 재작업은 작다.
- ~~앱 배포는 아직 안 했다~~ → **배포 완료**(2026-08-22). 다만 그 과정에서 위 「초록불이 거짓말」을 발견했다 — Cloudflare Pages 는 저장소를 **안 보고 있었다.**

### 🔴 별명 클레임을 켜려다 멈췄다 — 주석이 거짓 보증을 하고 있었다 (2026-08-22)

사용자가 `nick` 클레임을 켜라고 했다. 켜기 전에 **켰을 때 실제로 뭐가 나가는지** 재 봤다.

| 실측(라이브 41명) | |
|---|---|
| `hasSetNickname == true` | 41명 (전원 학생) |
| 그중 **`nickname` 이 `name` 과 글자 그대로 같음** | **27명 (66%)** |
| 닉네임 비어 있음 / 20자 초과 | 0명 / 0명 |

서버 주석은 이렇게 적혀 있었다 —
"학생이 **스스로 정한 닉네임**(hasSetNickname)일 때만 그 값이 닉네임임이 **보장**된다."
**거짓이었다.** 그 플래그가 보장하는 건 "학생이 직접 입력했다"뿐이고, 아이들은 자기 이름을 적는다.
그대로 켰으면 **학생 3분의 2의 실명이 외부 앱으로 나갔다.**

🔴 그리고 그 거짓 보증을 **내가 사용자에게 그대로 옮겼다** — "실명은 안 나간다"고 말하고
   승인을 받았다. 승인의 전제가 내가 준 틀린 사실이었으므로, 승인이 있어도 켤 수 없다.
   **주석을 근거로 사실을 주장하지 말 것. 주석은 검증되지 않는다.**

→ 한 것:
- **켜지 않았다.** `allowedClaims` 는 여전히 비어 있다.
- 판정을 `policy.chooseNickClaim` 순수 함수로 꺼내고 **이름과 같으면 안 싣도록** 고쳤다
  (공백만 다른 경우도 막는다). 인라인이면 소스 grep 으로밖에 못 지킨다 —
  실명이 나가는지를 가르는 판정은 **실제 입력을 넣어 결과를 보는 테스트**가 붙어야 한다.
- 옛 테스트가 정확히 그 grep 이었다(`toMatch(/hasSetNickname === true/)`) — 행동 테스트 7개로 교체.
  **변이 4종 전부 검출**(이름 대조 제거 · trim 제거 · 플래그 검사 제거 · 20자 자르기 제거).
- ⚠️ **완전한 방어가 아니라고 코드·테스트에 적었다.** `name` 이 비어 있거나 ID 인 학생의
  닉네임이 우연히 실명이면 여전히 나간다. 그래서 이 클레임은 앱별 심사 후에만 켠다.
- `aap-switch.mjs claims <appId> <none|nick|cls|nick,cls>` 추가 + `list` 에 클레임 열 표시
  (켜 놓고 잊으면 안 되는 종류라 목록에 보이게 했다).

📌 **그래서 지금 켤지는 다시 사용자 판단이다.** 가드를 넣은 뒤에도 14명만 별명이 나가고
   27명은 "연결됨"으로 뜬다. 얻는 것(배지 문구)이 그만한 값인지는 제가 정할 일이 아니다.
